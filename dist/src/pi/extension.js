...
export function parseArguments(args, cwd) {
    ...
    for (let i = 1; i < tokens.length; i++) {
        ...
        if (flag === "--json")
            out.json = true;
        else if (flag === "--workspace" || flag === "--session" || flag === "--result-bundle" || flag === "--expect-tests") {
            const value = tokens[++i];
            if (!value || value.startsWith("-"))
                throw new Error(`${flag} requires a value`);
            if (flag === "--expect-tests") {
                const n = Number(value);
                if (!Number.isSafeInteger(n) || n < 0)
                    throw new Error("--expect-tests must be a non-negative integer");
                out.expected = n;
            }
            else {
                if (!isSafePath(value))
                    throw new Error(`${flag} must be an absolute safe path`);
                if (flag === "--workspace")
                    out.workspace = value;
...
function abortable(operation, signal) {
    if (signal.aborted)
        return Promise.reject(new Error("status cancelled"));
...
function openAbortable(path, signal, operation) {
    if (signal.aborted)
        return Promise.reject(new Error("status cancelled"));
    return new Promise((resolve, reject) => {
        let cancelled = false;
...
export async function readPiSession(path, signal, io = { stat, open }) {
    if (signal.aborted)
        throw new Error("status cancelled");
    const info = await abortable(io.stat(path), signal);
    if (!info.isFile())
        throw new Error("Pi session is not a regular file");
    const handle = await openAbortable(path, signal, () => io.open(path, "r"));
    try {
        ...
        for (const raw of lines) {
            ...
            try {
                ...
                if (record.type === "message" && role === "user")
                    events.push('{"event":"task_started"}');
                if (record.type === "message" && role === "assistant") {
                    const stop = piMessage.stopReason;
                    if (stop === "stop" || stop === "end_turn")
                        events.push('{"event":"task_complete"}');
                    else if (stop === "aborted" || stop === "cancelled")
        ...
        if (malformedLines > 0) {
            parsed.malformedLines += malformedLines;
            parsed.diagnostics.push(`${malformedLines} malformed Pi JSONL line(s)`);
        }
        if (read.bytesRead > MAX_SESSION_BYTES) {
            parsed.truncated = true;
            parsed.diagnostics.push("Pi session exceeded byte bound");
        }
        return parsed;
    }
    finally {
...
export async function runStatus(options, signal) {
    if (signal.aborted)
        throw new Error("status cancelled");
...
function publicJson(receipt) { return JSON.stringify({ ...receipt, workspace: "<workspace>", session: { ...receipt.session, path: receipt.session.path ? "<session>" : undefined }, git: { ...receipt.git, detail: receipt.git.detail ? "git unavailable" : undefined }, diagnostics: receipt.diagnostics.map((item) => item.includes("unavailable") ? "evidence unavailable" : item.includes("bound") ? "input exceeded bound" : "evidence warning") }); }
export default function xcodeLoopExtension(pi) {
    pi.registerCommand("xcode-loop", { description: "Read-only Xcode loop evidence status", handler: async (args, ctx) => {
            try {
                const hostSignal = ctx.signal;
                if (hostSignal?.aborted) {
                    ctx.ui.notify("status cancelled", "error");
                    return;
                }
                const options = parseArguments(args, ctx.cwd);
                const session = options.session ?? ctx.sessionManager?.getSessionFile();
                const effective = session ? { ...options, session, piSession: options.session === undefined } : options;
                ...
                const safe = message === "status cancelled" || message === "status timed out" || message.startsWith("usage:") || message.startsWith("unknown argument") || message.startsWith("--");
                ctx.ui.notify(safe ? message : "xcode-loop status unavailable", "error");
            }
        } });
}
