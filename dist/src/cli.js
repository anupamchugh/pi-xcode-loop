...
function parse(args) {
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        if (arg === "--json")
            out.json = true;
        else if (["--workspace", "--session", "--result-bundle", "--expect-tests"].includes(arg ?? "")) {
            const value = args[++i];
            if (!value)
                throw new Error(`${arg} requires a value`);
            if (arg === "--workspace" || arg === "--session" || arg === "--result-bundle") {
                if (!isSafePath(value))
                    throw new Error(`${arg} must be an absolute safe path`);
                if (arg === "--workspace")
                    out.workspace = value;
                else if (arg === "--session")
                    out.session = value;
                ...
                out.expected = Number(value);
                if (!Number.isInteger(out.expected) || out.expected < 0)
...
async function main() {
    try {
        ...
        if (options.json)
            ...
            console.log(`${receipt.verdict}: session=${receipt.session.state} git=${receipt.git.state} tests=${receipt.tests.state} (${receipt.tests.executed} executed)`);
