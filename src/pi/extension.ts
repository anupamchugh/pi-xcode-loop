...
export function parseArguments(args: string, cwd: string): Options {
  ...
  for (let i = 1; i < tokens.length; i++) { const flag = tokens[i]; if (flag === "--json") out.json = true; else if (flag === "--workspace" || flag === "--session" || flag === "--result-bundle" || flag === "--expect-tests") { const value = tokens[++i]; if (!value || value.startsWith("-")) throw new Error(`${flag} requires a value`); if (flag === "--expect-tests") { const n = Number(value); if (!Number.isSafeInteger(n) || n < 0) throw new Error("--expect-tests must be a non-negative integer"); out.expected = n; } else { if (!safePath.test(value)) throw new Error(`${flag} must be an absolute safe path`); if (flag === "--workspace") out.workspace = value; else if (flag === "--session") out.session = value; else out.result = value; } } else throw new Error(`unknown argument: ${flag}`); }
  return out;
}
async function readPiSession(path: string, signal: AbortSignal): Promise<ReturnType<typeof parseSessionLog>> {
  if (signal.aborted) throw new Error("status cancelled"); const info = await stat(path); if (!info.isFile()) throw new Error("Pi session is not a regular file"); const handle = await open(path, "r");
  try { const buffer = Buffer.allocUnsafe(MAX_SESSION_BYTES + 1); const read = await handle.read(buffer, 0, MAX_SESSION_BYTES + 1, 0); if (signal.aborted) throw new Error("status cancelled"); const lines = buffer.subarray(0, Math.min(read.bytesRead, MAX_SESSION_BYTES)).toString("utf8").split(/\r?\n/); const events: string[] = []; for (const line of lines) { try { const value = JSON.parse(line) as Record<string, unknown>; const message = value.message as Record<string, unknown> | undefined; const role = message?.role; if (value.type === "message" && role === "user") events.push('{"event":"task_started"}'); if (value.type === "message" && role === "assistant") { const stop = message?.stopReason; if (stop === "stop" || stop === "end_turn") events.push('{"event":"task_complete"}'); else if (stop === "aborted" || stop === "cancelled") events.push('{"event":"cancelled"}'); else if (stop === "error") events.push('{"event":"error"}'); } } catch { /* Pi records unrelated to messages are ignored. */ } } const parsed = parseSessionLog(events.join("\n")); if (read.bytesRead > MAX_SESSION_BYTES) { parsed.truncated = true; parsed.diagnostics.push("Pi session exceeded byte bound"); } return parsed; }
