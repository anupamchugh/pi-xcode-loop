...
async function main() {
  try {
    const [command, ...args] = process.argv.slice(2); if (command !== "status" || args.includes("--help") || args.includes("-h")) { if (args.includes("--help") || args.includes("-h")) { console.log(help()); return; } throw new Error("expected: status"); }
