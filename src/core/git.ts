...
export async function readGitSnapshot(workspace: string): Promise<GitSnapshot> {
  try {
    const [branch, commit, status] = await Promise.all([
      run("git", ["-C", workspace, "rev-parse", "--abbrev-ref", "HEAD"], { maxBuffer: 32_000 }),
      run("git", ["-C", workspace, "rev-parse", "HEAD"], { maxBuffer: 32_000 }),
      run("git", ["-C", workspace, "status", "--porcelain=v2"], { maxBuffer: 128_000 })
    ]);
